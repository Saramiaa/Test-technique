...
window.onload = function(){
  ...
  // heure récup, non dispo dans JSON
  function hour()
  {
    var today = new Date();
  ...
  function ordersTemplate(e)
  {
    ...
    '<li> <i class="fas fa-clock fa-2x"></i> ' + hour() + '</li>' +
  ...
  function ordersDetail(items) {
    ...
    if (items.id == id) {
      ...
      for(var i = 0; i < items.items.length; i++){
        ...
        order.push('<li class="list-detail" style = "color:'+ color +'">' + name + ' ' + price + currency);

      }

      return '<h1>Detail de la table N°'+items.table+'</h1><ul class="bon">' + order +'</ul><h3>Montant total à régler : <span>'+total(items.items)+' €</span></h3>';
